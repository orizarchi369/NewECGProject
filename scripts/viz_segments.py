...
class ECGSegments(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # mask-from-meta (for plotting we still show full length)
        L = lab.shape[0]
...
class ConvBlock1D(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.net = nn.Sequential(nn.Conv1d(in_ch, out_ch, 3, padding=1),
                                 nn.ReLU(inplace=True))
    def forward(self, x): return self.net(x)

class UNet1D_MTL(nn.Module):
    def __init__(self, K=len(RHYTHMS)):
        super().__init__()
        self.K = K
        self.enc1 = ConvBlock1D(1+K, 32); self.pool=nn.MaxPool1d(2,2)
        self.enc2 = ConvBlock1D(32,64); self.enc3=ConvBlock1D(64,128)
        self.up1=nn.ConvTranspose1d(128,64,2,2); self.dec1=ConvBlock1D(64+64,64)
        self.up2=nn.ConvTranspose1d(64,32,2,2);  self.dec2=ConvBlock1D(32+32,32)
        self.out=nn.Conv1d(32,4,1)
        self.cls_pool=nn.AdaptiveAvgPool1d(1); self.cls_fc=nn.Linear(128,K)
    ...
    def forward(self, x, cond):
        B,_,L=x.shape
        cond = cond.unsqueeze(-1).expand(-1,-1,L)
        x=torch.cat([x,cond],1)
        e1=self.enc1(x); e2=self.enc2(self.pool(e1)); e3=self.enc3(self.pool(e2))
        d1=self.up1(e3); e2a=F.interpolate(e2, d1.size(2))
        d1=self.dec1(torch.cat([d1,e2a],1))
        d2=self.up2(d1); e1a=F.interpolate(e1, d2.size(2))
        d2=self.dec2(torch.cat([d2,e1a],1))
        return self.out(d2)  # [B,4,L]

def make_cmap():
    # 0 bg white, 1 P orange, 2 QRS red, 3 T blue
...
def plot_example(sig, lab, pred, msk, out_png, title):
    ...
    ax0.plot(np.arange(L), sig, linewidth=1.0)
    ax0.set_xlim(0, L-1); ax0.set_ylabel('mV (norm)'); ax0.set_title(title)
    ax0.grid(True, alpha=0.2)

    # GT bands
    ax1 = fig.add_subplot(gs[1,0])
    ax1.imshow(lab[None,:], aspect='auto', cmap=make_cmap(), vmin=0, vmax=3)
    ax1.set_yticks([]); ax1.set_ylabel('GT')

    # Pred bands
    ax2 = fig.add_subplot(gs[2,0])
    ax2.imshow(pred[None,:], aspect='auto', cmap=make_cmap(), vmin=0, vmax=3)
    ax2.set_yticks([]); ax2.set_ylabel('Pred')
    for ax in [ax1, ax2]:
        ax.set_xlim(0, L-1)
...
def main():
    ...
    model = UNet1D_MTL().to(device).eval()
